...
filter, and assessing them exhausted the free tier of Gemini AND Groq —

    ! gemini: HTTP 429: You exceeded your quota
    ! groq:   HTTP 429: Rate limit reached
    LLM assessed: 60   (48 fell back to keywords)

— every run, re-buying verdicts it had already paid for the day before. These
tests are what stop that coming back.

Every HTTP call is mocked. No test touches a live API.
"""
...
def gemini_reply(*job_ids: str, score: int = 85, **overrides) -> httpx.Response:
    entries = []
...
class TestCacheHits:
    ...
    async def test_a_second_run_makes_no_llm_calls(self, pipe, db, job_factory, summary_factory):
        ...
        route = respx.post(GEMINI_URL).mock(return_value=gemini_reply("j1"))
    ...
    async def test_only_the_new_jobs_are_judged(self, pipe, db, job_factory, summary_factory):
        respx.post(GEMINI_URL).mock(return_value=gemini_reply("j1"))
        ...
        route = respx.post(GEMINI_URL).mock(return_value=gemini_reply("j2"))
    ...
    async def test_an_edited_posting_is_judged_again(self, pipe, db, job_factory, summary_factory):
        ...
        job.content_hash = "h2"
        route = respx.post(GEMINI_URL).mock(return_value=gemini_reply("j1", score=20))
        before = route.call_count
        summary = summary_factory()
...
class TestCacheWrites:
    ...
    async def test_a_verdict_for_an_unstored_job_is_dropped_not_raised(
        ...
        respx.post(GEMINI_URL).mock(return_value=gemini_reply("ghost"))
    ...
    async def test_a_cached_verdict_survives_a_round_trip_intact(
        ...
        respx.post(GEMINI_URL).mock(
            return_value=gemini_reply("j1", german_required=True, country="Austria")
        )
        job = job_factory(id="j1", content_hash="h1")
        await pipe._assess_with_llm([job], summary_factory())
        db.upsert(job)
